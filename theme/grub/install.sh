...
main() {
  ...
  then
    ...
    cp -rf blur-grub2-hd /boot/grub/themes/
  else
    echo -e "\e[1m\e[32m==> \e[97mCopying files...\e[0m"
    cp -rf blur-grub2-hd /boot/grub2/themes/
  fi
      ...
      if [ "$answer" = "y" ];then
        if [ -d "/boot/grub" ];then
            i=$(sed -n 's,^#\?GRUB_THEME=,&,p' /etc/default/grub)
                if [[ -z $i ]]; then
                    echo -e "\nGRUB_THEME=/boot/grub/themes/blur-grub2-hd/theme.txt" >> /etc/default/grub
                else
                    sed "s,^#\?GRUB_THEME=.*,GRUB_THEME=/boot/grub/themes/blur-grub2-hd/theme.txt," </etc/default/grub >/etc/default/grub.~
          ...
          break
            ...
            i=$(sed -n 's,^#\?GRUB_THEME=,&,p' /etc/default/grub)
                if [[ -z $i ]]; then
                    echo -e "\nGRUB_THEME=/boot/grub2/themes/blur-grub2-hd/theme.txt" >> /etc/default/grub
                else
                    sed "s,^#\?GRUB_THEME=.*,GRUB_THEME=/boot/grub2/themes/blur-grub2-hd/theme.txt," </etc/default/grub >/etc/default/grub.~
                    mv /etc/default/grub.~ /etc/default/grub
                fi
          echo -e "\e[1m\e[32m==> \e[97mApplying changes...\e[0m"
